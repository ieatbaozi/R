...
query.all <- paste0("select MIT..PowerMeterData.PMMID,MIT..PowerMeterMaster.MeterName,((kWh_import_H+kWh_import_L)/1000) as 'kWh',MIT..PowerMeterData.AddDate
                    ...
                    where MIT..PowerMeterMaster.MeterName like 'MDB%' and MIT..PowerMeterData.AddDate between ('2017-01-01') and (GETDATE()) 
...
x<-as.character(as.POSIXct(x) - 3600)
y<-as.character(as.POSIXct(y) + 3600)
meter_all <- meter_all[(meter_all$DateTime>=x & meter_all$DateTime<=y),]
