...
b <- as.character(as.POSIXct(y) + 7199)
query.all <- paste0("select MIT..PowerMeterData.PMMID,MIT..PowerMeterMaster.MeterName,((kWh_import_H+kWh_import_L)/1000) as 'kWh',MIT..PowerMeterData.AddDate
                    ...
                    where MIT..PowerMeterMaster.MeterName like 'MDB%' and MIT..PowerMeterData.AddDate between ('",x,"') and ('",b,"') 
...
#write.csv(meter_all, file = paste0("Report",x,"to",y,".csv"),row.names=FALSE)

#ggplot(meter_all,aes(x=meter_all$DateTime,y=meter_all$`MDB1-1`,ymin=0),xaxt="n") + 
...
#weekday.partmean <- aggregate( meter_all$`MDB1-1` ~ weekdays(meter_all$DateTime) , meter_all , mean )
...
ggplot(meter_all) + 
  geom_line(aes(x = DateTime,y=`MDB1-1`,col=`MDB1-1`),lwd=1.2) +
  ...
  theme(text = element_text(size=10),
        ...
        panel.grid.minor.x = element_line(color = 'grey96', size = .01))



#library(AnomalyDetection)

#df1 <- meter_all[,c(1,15)]
 #test <- AnomalyDetectionTs(df1, max_anoms=0.09, direction='neg', plot=TRUE)
 #test$plot
