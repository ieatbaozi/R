...
query.all <- paste0("select MIT..PowerMeterData.PMMID,MIT..PowerMeterMaster.MeterName,((kWh_import_H+kWh_import_L)/1000) as 'kWh',MIT..PowerMeterData.AddDate
                    ...
                    where MIT..PowerMeterMaster.MeterName like 'MDB%' and MIT..PowerMeterData.AddDate between ('2017-04-01') and (GETDATE()) 
...
write.csv(meter.all, file = paste0("Report",x,"to",y,".csv"),row.names=FALSE)

ggplot(meter.all) + ggtitle("TotalPower") +
  geom_line(aes(x = DateTime,y=TotalPower,col=TotalPower),lwd=1.2) +
...
p <- ggplot(meter.all) + ggtitle("TotalPower") +
  geom_line(aes(x = DateTime,y=TotalPower),lwd=0.8) +
  geom_vline(data = weeks, 
             aes(xintercept = as.numeric(w)),
             color = 'grey55', size = 1) +
  scale_x_datetime(labels = date_format("%m-%d"),
...
ggplotly(p) %>% config(displayModeBar = FALSE,xaxis = list(title="DateTime"))
