...
ui <- fluidPage(
   ...
   mainPanel(
     ...
     dateRangeInput("daterange", "Date range:",
                    start = Sys.Date()-7,
                    end = Sys.Date(),min=start.date,max=Sys.Date()),
     actionButton("do", "Submit"),
     navbarPage(
       title = 'Options',
       tabPanel('Display Table',  DT::dataTableOutput('ex1')),
       tabPanel('Display Plot',   plotOutput("plot", click = "plot_click"),
...
server <- function(input, output) {
    ...
    a<-paste(x,'07:00:00')
    b<-paste(y,'07:00:00')
    meter.all <- meter_all[(meter_all$DateTime>=a & meter_all$DateTime<=b),]
    ...
    plotInput <- reactive({
      ggplot(meter.all,main = input$dataset) + ggtitle(input$dataset) + ylab(input$dataset) +
        geom_line(aes(x = DateTime,y=meter.all[,input$dataset],col=meter.all[,input$dataset]),lwd=1.2) +
